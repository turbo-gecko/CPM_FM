...
class SerialManager:
    ...
    def open_port(self, port_type: str, settings: dict) -> bool:
        ...
        try:
            ...
            s = settings.get('serial', settings) if isinstance(settings, dict) and 'serial' in settings else settings
            ...
            if self.terminal_port and self.terminal_port.is_open:
                try:
                    if self.terminal_port.in_waiting > 0:
                        data = self.terminal_port.read(self.terminal_port.in_waiting).decode('ascii', errors='replace')
