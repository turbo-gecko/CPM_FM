...
| ID | Requirement | Priority | Verification | Source |
|----|-------------|----------|--------------|--------|
| FR-090 | All data received from the Terminal Port shall be accumulated in a receive data buffer that is retained until explicitly cleared by the Terminal Window Clear button (FR-095). Local-echo text (FR-093) is not received data and shall not enter this buffer. | Mandatory | T | App_Design §Receiving data; impl. `mw_remote.py:handle_terminal_recv`, `_rx_buffer` |
| FR-091 | All data received from the Terminal Port shall be displayed in the receive text area of the Terminal Window. | Mandatory | T | App_Design §Receiving data; impl. `app.py:_on_term_write`, `mw_remote.py:handle_terminal_recv`, `terminal_window.py:write_text`, `serial_manager.py:_read_loop`; tests `test_serial_manager.py` |
| FR-092 | All data transmitted to the Terminal Port (including the appended EOL, FR-094) shall be accumulated in a transmit data buffer that is retained until explicitly cleared by the Terminal Window Clear button (FR-095). | Mandatory | T | App_Design §Sending data; impl. `mw_remote.py:handle_terminal_send`, `_tx_buffer` |
| FR-093 | When the Local Echo checkbox is enabled, transmitted data shall be copied to the receive text area of the Terminal Window. | Mandatory | T | App_Design §Sending data; impl. `app.py:_on_term_write`, `mw_remote.py:_set_local_echo`, `mw_remote.py:handle_terminal_send` |
| FR-094 | Transmitted data shall have the configured EOL character(s) appended before being sent, except when the transmit field resolves to a bare control character (FR-156), in which case the control byte is sent on its own with no appended EOL. *(v2.12: added the bare-control-character exception.)* | Mandatory | T | App_Design §Sending data; impl. `mw_remote.py:handle_terminal_send`, `terminal_window.py:send_text` |
| FR-095 | When the Clear button in the Terminal Window is pressed, the receive text area shall be cleared, and the receive and transmit data buffers (FR-090, FR-092) shall be cleared. | Mandatory | T | App_Requirements §Terminal Window; impl. `terminal_window.py:clear_text`, `mw_remote.py:clear_terminal_buffers` |
