...
| FR-091 | All data received from the Terminal Port shall be interpreted as a VT-100/ANSI terminal stream and rendered on the Terminal Window's character-cell screen (the receive area) — including cursor positioning, erase, and character attributes (FR-157). The default screen geometry is 80 columns × 24 rows with scrollback. *(v2.17: was a plain-text append; now a VT-100 screen model.)* | Mandatory | T | impl. `terminal/vt100_engine.py:VT100Engine`, `gui/terminal_view.py:TerminalView`, `app.py:_on_term_write`, `mw_remote.py:handle_terminal_recv`, `serial_manager.py:_read_loop`; tests `test_vt100_engine.py`, `test_terminal_view.py` |
...
| FR-093 | When the Local Echo checkbox is enabled, transmitted data shall be copied to the receive area of the Terminal Window (rendered through the engine, FR-091). | Mandatory | T | App_Design §Sending data; impl. `app.py:_on_term_write`, `mw_remote.py:_set_local_echo`, `mw_remote.py:handle_terminal_send`, `mw_remote.py:handle_terminal_key` |
...
| FR-155 | *Removed in v2.17.* (Was: with the transmit field empty, Send/Enter transmits a bare EOL.) The transmit field was removed with the interactive-keyboard migration; pressing Enter in the Terminal Window now transmits the configured EOL directly (FR-096/FR-094/FR-158). | Mandatory | — | superseded by FR-096, FR-158 |
| FR-156 | *Removed in v2.17.* (Was: the transmit field interpreted caret notation `^A`..`^?` for control characters.) The transmit field was removed; control characters are now produced by the corresponding keys and Ctrl-combinations, encoded live (FR-158). | Mandatory | — | superseded by FR-158 |
| FR-157 | The terminal shall interpret the VT-100/ANSI control and escape sequences emitted by the remote — cursor positioning and movement, erase-in-line and erase-in-display, SGR character attributes including ANSI colour, the scrolling region, tab stops, and the DEC special-graphics (line-drawing) character set — and render the result on the screen (FR-091). Received bytes are decoded UTF-8 by default (invalid bytes replaced) with a raw 8-bit mode available for legacy output. Application-cursor-key mode (DECCKM) is not tracked; arrow keys always send the normal-mode sequences (FR-158). *(v2.17.)* | Mandatory | T | impl. `terminal/vt100_engine.py:VT100Engine`; tests `test_vt100_engine.py` |
| FR-158 | Keys typed into the Terminal Window shall be encoded to VT-100 byte sequences and transmitted per keystroke (FR-096): printable characters as UTF-8; Enter as the configured EOL (FR-094); Backspace as 0x08; Tab as 0x09; Escape as 0x1B; the arrow, Home, End, Page Up/Down, Insert, Delete and F1–F12 keys as their CSI/SS3 sequences; and Ctrl-letter and Ctrl-`[`/`\`/`]`/Space as the corresponding C0 control bytes. *(v2.17.)* | Mandatory | T | impl. `gui/terminal_view.py:encode_key`, `gui/terminal_view.py:keyPressEvent`; tests `test_terminal_view.py` |
...
| UIR-062 | The Receive view shall maintain scrollback and shall auto-scroll to the newest output when the Autoscroll control (UIR-066) is enabled. | Mandatory | D | App_Requirements §Terminal Window; impl. `gui/terminal_view.py:TerminalView`, `gui/terminal_view.py:refresh`, `gui/terminal_view.py:set_autoscroll` |
