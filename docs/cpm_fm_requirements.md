...
| FR-023 | When the Help > Manual menu item is selected, the application shall present the user manual in the Manual Window (UIR-091). If the window is already open, it shall be raised and activated rather than opening a second copy. *(v2.13.)* | Mandatory | T | impl. `app.py:menu_manual`, `gui/manual_dialog.py:ManualDialog` |
...
| FR-120 | The user shall be able to cancel a file transfer while it is in progress. The Transfer Progress Dialog (UIR-051) shall provide a **Cancel** button; pressing it shall request cancellation of the current transfer. On a cancellation request the application shall: signal the active X-Modem transfer to abort, transmitting the X-Modem CAN (0x18) sequence on the Transport Port so the remote program (PCGET/PCPUT) also aborts (NFR-003m–o) and flushing the Transport Port in **both** directions as part of the abort — discarding the partially-transmitted packet still queued for transmit (so it does not keep draining to the remote after the cancel) and dropping any data the remote was still sending — so the cancellation takes effect immediately rather than the transfer appearing to continue until the serial buffers empty; stop the batch so that no further files are attempted (the multi-file analogue of FR-108); close the progress dialog; and set the status bar to a cancellation message. If one or more files of a multi-file batch (FR-106) completed before the cancellation, the destination list shall be refreshed once (per FR-099); a file that was only partially received shall not be written to the host. Cancellation is initiated on the GUI thread and observed by the transfer worker thread via a thread-safe flag, with the dialog teardown marshalled back to the GUI thread (NFR-004). The abort's wait for the queued CAN bytes to leave the port shall be **bounded** so that a transmit line that cannot drain (flow control de-asserted by the aborting remote) can never block the transfer worker — which would otherwise never signal completion and would leave the progress dialog stuck open with its Cancel button disabled and no way to close it. The other blocking waits on the transfer worker thread — the post-launch handshake delay (FR-089), the between-files settle (FR-109), and the pre-upload remote listing read (FR-145) — shall likewise observe the cancel flag and wake promptly rather than running their full interval, so a Cancel pressed during any of them takes effect without waiting out the delay (a worst case of several seconds). *(v1.9; v2.13.1: the abort flushes the Transport Port transmit and receive buffers so the cancel takes effect at once; v2.13.2: the wait for the CAN bytes to drain is bounded so a flow-control stall cannot hang the worker and freeze the progress dialog open, and the worker-thread launch/settle/listing waits became cancellable so a Cancel during them is observed promptly.)* | Mandatory | T | impl. `app.py:_request_transfer_cancel`, `_on_transfer_cancelled`, `_transfer_cancel` flag, `transfer_cancelled` signal, batch drivers; `gui/transfer_dialog.py` Cancel button; `xmodem.py:send_file`, `receive_file` cancel checks, `_abort`, `_drain_tx` |
...
| ID | Requirement | Priority | Verification | Source |
|----|-------------|----------|--------------|--------|
| UIR-091 | The Manual Window (FR-023) shall be a non-modal, resizable window titled "User Manual" that displays the bundled user manual (DR-047) rendered from Markdown to HTML in a scrollable, read-only text view. Headings shall carry GitHub-style anchors so the manual's table-of-contents links navigate within the document; external `http(s)` links shall open in the host's default browser. The window shall provide a single **Close** button (centred per UIR-075) that dismisses it. If the manual file cannot be read, the window shall display an explanatory message rather than failing to open. *(v2.13.)* | Mandatory | T | impl. `gui/manual_dialog.py:ManualDialog`, `render_manual_html`; `lang/*.txt` (`manual.title`, `manual.load_error`, `button.close`); FR-023, DR-047 |
...
| ID | Requirement | Priority | Verification | Source |
|----|-------------|----------|--------------|--------|
| DR-047 | The user manual (FR-023) shall ship with the application as a UTF-8 Markdown file `cpm_fm_manual.md` located inside the package at `src/cpm_fm/docs/`, resolved at run time relative to the module file so the same lookup works from source, from an installed wheel (shipped as package data), and from a frozen (PyInstaller) bundle, where the file is placed at `cpm_fm/docs/`. The file shall be rendered to HTML for display (UIR-091) using the `markdown` library with the `toc`/`tables`/`fenced_code`/`attr_list`/`sane_lists` extensions and a GitHub-compatible heading slugify. A missing or unreadable file shall not block opening the Manual Window (UIR-091); the window shall instead show an explanatory message. *(v2.13.)* | Mandatory | T | impl. `gui/manual_dialog.py` (`MANUAL_PATH`, `load_manual_markdown`, `render_manual_html`); `_pyinstaller_common.py:build_datas`/`HIDDEN_IMPORTS`; `pyproject.toml` (`markdown` dependency, `docs/*.md` package data) |
