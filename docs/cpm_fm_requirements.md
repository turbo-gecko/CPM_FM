...
| ID | Requirement | Priority | Verification | Source |
|----|-------------|----------|--------------|--------|
| FR-110 | The Host Files list (UIR-011) shall provide a right-click context menu (UIR-018) offering the actions **To Remote** (FR-119), **View/Edit**, **Rename**, and **Delete**, each operating on the file the user right-clicked. *(v1.8; To Remote added v1.8.1.)* | Mandatory | T | impl. `app.py:_host_context_menu`, `_host_to_remote`, `_host_view`, `_host_rename`, `_host_delete` |
| FR-111 | The Remote Files list (UIR-012) shall provide a right-click context menu (UIR-019) offering the actions **To Host** (FR-119), **View**, **Rename**, and **Delete**, each operating on the file the user right-clicked. *(v1.8; To Host added v1.8.1.)* | Mandatory | T | impl. `app.py:_remote_context_menu`, `_remote_to_host`, `_remote_view`, `_remote_rename`, `_remote_delete` |
...
| FR-115 | The Delete action (host FR-110, remote FR-111) shall present a modal File Action Dialog (UIR-057) showing the selected file's name in a **read-only** field, with an **Apply** button (confirm deletion) and a **Cancel** button (no change). *(v1.8.)* | Mandatory | T | impl. `app.py:_host_delete`, `_remote_delete`, `file_action_dialog.py:FileActionDialog` |
| FR-116 | For **host** files, the Rename action shall rename the file on the host filesystem using a standard filesystem rename, and the Delete action shall delete the file using a standard filesystem delete. On failure, an error dialog shall report the cause and the file list shall be left unchanged. *(v1.8.)* | Mandatory | T | impl. `app.py:_host_rename`, `_host_delete` |
| FR-117 | For **remote** files, the Rename action shall send the configured `rename_remote_cmd` (UIR-055, default `REN $2=$1`) on the Terminal Port, with `$1` replaced by the original filename and `$2` by the new filename; the Delete action shall send the configured `delete_remote_cmd` (UIR-056, default `ERA $1`) on the Terminal Port, with `$1` replaced by the filename. If the configured command is empty, no command is sent. The command shall not be issued when the Terminal Port is closed (the status bar shall read "Terminal port not open - cannot rename"/"...cannot delete"). *(v1.8.)* | Mandatory | T | impl. `app.py:_remote_rename`, `_remote_delete`, `_do_remote_file_cmd` |
| FR-118 | After a successful Rename or Delete, the application shall refresh the affected file list so the change is reflected without manual intervention: a host action refreshes the Host Files list (FR-060); a remote action refreshes the Remote Files list (the FR-074–FR-079 process). *(v1.8.)* | Mandatory | T | impl. `app.py:_host_rename`, `_host_delete`, `_do_remote_file_cmd` |
| FR-119 | The Host Files context menu (FR-110) shall provide a **To Remote** action and the Remote Files context menu (FR-111) a **To Host** action, each initiating an X-Modem transfer of the single file under the cursor: **To Remote** copies the host file to the remote system, and **To Host** copies the remote file to the host. Each action shall behave exactly as the corresponding Copy to Remote / Copy to Host button (FR-080–FR-087, FR-105, FR-099) applied to that one file — including the requirement that **both** the Terminal and Transport status flags be connected (FR-080/CR-010), the modal progress dialog (FR-105), and the automatic refresh of the destination list on success (FR-099). If both flags are not connected, an error dialog with the body text "Transport port not connected" shall be shown and no transfer shall occur. *(v1.8.1.)* | Mandatory | T | impl. `app.py:_host_to_remote`, `_remote_to_host`, `_transfer_to_remote_batch`, `_transfer_to_host_batch` |
...
| UIR-018 | The Host Files multi-select widget (UIR-011) shall present a context menu on right-click containing the items **To Remote** (FR-119), **View/Edit**, **Rename**, and **Delete** (FR-110), with **To Remote** at the top of the menu separated from the file actions. The menu shall act on the file under the cursor. *(v1.8; To Remote added v1.8.1.)* | Mandatory | I | impl. `app.py:setup_layout`, `_host_context_menu` |
| UIR-019 | The Remote Files multi-select widget (UIR-012) shall present a context menu on right-click containing the items **To Host** (FR-119), **View**, **Rename**, and **Delete** (FR-111), with **To Host** at the top of the menu separated from the file actions. The menu shall act on the file under the cursor. *(v1.8; To Host added v1.8.1.)* | Mandatory | I | impl. `app.py:setup_layout`, `_remote_context_menu` |
...
| UIR-055 | The dialog shall provide a "Rename Remote" text field limited to 79 characters with a default value of `REN $2=$1`, persisted as the `rename_remote_cmd` setting, specifying the remote command used to rename a remote file (FR-117). `$1` denotes the original filename and `$2` the new filename. *(v1.8.)* | Mandatory | T | impl. `config_dialogs.py:GeneralConfigDialog` |
| UIR-056 | The dialog shall provide a "Delete Remote" text field limited to 79 characters with a default value of `ERA $1`, persisted as the `delete_remote_cmd` setting, specifying the remote command used to delete a remote file (FR-117). `$1` denotes the filename. *(v1.8.)* | Mandatory | T | impl. `config_dialogs.py:GeneralConfigDialog` |
